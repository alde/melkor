...
REPO=github.com/alde/melkor
VERSION?=$(shell git describe HEAD --always | sed s/^v//)
DATE?=$(shell date -u '+%Y-%m-%d_%H:%M:%S')
DOCKERNAME?=alde/melkor
DOCKERTAG?=${DOCKERNAME}:${VERSION}
LDFLAGS=-X ${REPO}/version.Version=${VERSION} -X ${REPO}/version.BuildDate=${DATE}
SRC=$(shell find . -name '*.go')
TESTFLAGS="-v"

DOCKER_GO_SRC_PATH=/go/src/github.com/alde/melkor
DOCKER_GOLANG_RUN_CMD=docker run --rm -v "$(PWD)":$(DOCKER_GO_SRC_PATH) -w $(DOCKER_GO_SRC_PATH) golang:1.7 bash -c
...
melkor: ${SRC}
	go build -ldflags "${LDFLAGS}" -o $@ github.com/alde/melkor/cmd/melkor

docker/melkor: ${SRC}
	CGO_ENABLED=0 GOOS=linux go build -ldflags "${LDFLAGS}" -a -installsuffix cgo -o $@ github.com/alde/melkor/cmd/melkor

docker: docker/melkor docker/Dockerfile
	docker build -t ${DOCKERTAG} docker

docker-clean: docker/Dockerfile
	# Create the docker/melkor binary in the Docker container using the
	# golang docker image. This ensures a completely clean build.
	$(DOCKER_GOLANG_RUN_CMD) "make docker/melkor"
	docker build -t ${DOCKERTAG} docker
